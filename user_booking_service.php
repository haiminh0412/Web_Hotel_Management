...
<style>
    ...
    $select_user = "SELECT * FROM `information_booking` WHERE user_booking = '$username'";
    ...
    if ($res->num_rows > 0) {
        ...
        while ($row = $res->fetch_assoc()) {
            ...
            $date_in = new DateTime($row['date_booking']);
            $date_out = new DateTime($row['date_checkout']);
            $date = $date_in->diff($date_out)->days;
            $query = "SELECT price FROM service_event_room WHERE room_name = '{$row['room_name']}'";
            $res1 = DBHelper::executeResult($query);
            $get_row = $res1->fetch_assoc();
            $price = $get_row['price'];
            $sum_price = $date * $price;
            echo "<td>" .$sum_price .  "</td>";
            echo "<td><a href='delete_form_service.php?id=" . $row['STT'] . "' onclick='return confirmDelete()'>Hủy Dịch Vụ</a> | <a href='edit_form_service.php?id=" . $row['STT'] . "'>Sửa Thông Tin</a></td>";
