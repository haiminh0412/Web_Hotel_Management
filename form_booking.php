...
<body>
    ...
    if (isset($_GET['id'])) {
        ...
        $query = "select * from room inner join roomtype on room.RoomTypeId = roomtype.RoomTypeId and room.RoomId = '$id'";
        $result = DBHelper::execute($query);
    ...
    if (isset($_SESSION['check-in'])) {
        $checkIn = new DateTime($_SESSION['check-in']);
    }
    
    if (isset($_SESSION['check-out'])) {
        $checkOut = new DateTime($_SESSION['check-out']);
    }
    
    $interval = $checkIn->diff($checkOut);
    $khoang_cach = $interval->days;

    $checkIn = $checkIn->format('Y-m-d');
    $checkOut = $checkOut->format('Y-m-d');
    
    $gia_tien = $khoang_cach * $pricePerNight;
    ...
    <div class="container">
        <h2>Đặt Phòng</h2>
        <form id="booking-form" method="POST" action="ProcessBooking.php?id=<?php echo $id ?>" onsubmit="return checkBooking();">
            ...
            <div class="form-group">
                ...
                <input type="number" id="guests" name="guests" value="1" min="1" max="<?php echo $guests ?>" oninput="checkVaildGuests()">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="total-price" name="total-price" value="<?php echo number_format($gia_tien, 0, '.', ',') . 'đ' ?>" required readonly>
            </div>
        ...
        <script>
            ...
            window.onload = function() {
                ...
                document.getElementsByName("check-in")[0].setAttribute('min', checkIn);
